...
# how to get all reaction fluxes inside peroxisome that are > 0:
# from itertools import chain
# s = model.optimize()
# [s.fluxes[k.id] for k in list(chain.from_iterable([list(i.reactions) for i in model.metabolites 
# if i.compartment == "p"])) if s.fluxes[k.id]>0]
...
def simple_run():
    ...
    Acetoacetyl_CoA = model.metabolites.get_by_id("s_2914")
    #Metabolite(id="s_4270", formula="C25H36N7O18P3S", name="acetoacetyl-CoA[p]", charge=-4, compartment="p") #the original model's last metabolite has the id s_4269
    ...
    Hydrogen = model.metabolites.get_by_id("s_0801")
    ...
    model.add_reactions(reactionlist)
    return model
